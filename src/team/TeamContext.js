...
export const TeamProvider = ({children}) => {
    ...
    const handleAddMember = (hero) => {
        if (team.length === 6) {
            return Swal.fire({
                icon: 'error',
                title: '6 Members max! ',
                text: 'Remove someone to add another member'
              })
        }
        ...
        if (hero.biography.alignment === "good") {
            if (checkMaxGood(team)) {
                return Swal.fire({
                    icon: 'error',
                    text: 'Max of 3 good alignment heros'
                  })
            }
        }
        ...
        if (hero.biography.alignment === "bad") {
            if (checkMaxBad(team)) {
                return Swal.fire({
                    icon: 'error',
                    text: 'Max of 3 bad alignment heros'
                  })
            }
        }
    ...
    return (
        <TeamContext.Provider value={{team, dispatch, handleAddMember, handleRemoveMember}}>
            {children}
